...
var src = __webpack_require__(237);
var src_default = /*#__PURE__*/__webpack_require__.n(src);

// CONCATENATED MODULE: ./src/api/utils/debug.ts

const log = src_default()("run-if-diff");
function formatFileListForLog(statusByFile) {
...
function diff({ ref, files }) {
    log(`ref: ${ref}`);
    log(`files: ${Object.entries(files).length} files${formatFileListForLog(files)}\n`);
}

// CONCATENATED MODULE: external "child_process"
const external_child_process_namespaceObject = require("child_process");;
// CONCATENATED MODULE: ./src/api/utils/shell.ts
...
// CONCATENATED MODULE: ./src/api/utils/git.ts
...
function hasOneOrMoreTags({ cwd }) {
    return __awaiter(this, void 0, void 0, function* () {
        const { stdout } = yield exec("git", ["tag"], { cwd });
        return stdout.trim() !== "";
    });
}
function getMostRecentTag({ cwd }) {
    return __awaiter(this, void 0, void 0, function* () {
        const { stdout } = yield exec("git", ["describe", "--tags", "--abbrev=0"], { cwd });
        return stdout.split("\n")[0];
    });
}
function getFirstCommit({ cwd }) {
    return __awaiter(this, void 0, void 0, function* () {
        const { stdout } = yield exec("git", ["rev-list", "--max-parents=0", "HEAD"], { cwd });
        return stdout.split("\n")[0];
    });
}
function getDefaultRef({ cwd, }) {
    return __awaiter(this, void 0, void 0, function* () {
        const hasTags = yield hasOneOrMoreTags({ cwd });
        if (hasTags) {
            log("the repo has tags, defaulting to the most recent tag");
            return yield getMostRecentTag({ cwd });
        }
        else {
            log("the repo has no tags, defaulting to the first commit");
            return yield getFirstCommit({ cwd });
        }
    });
}
...
(function (DiffStatus) {
    ...
    DiffStatus["ChangedType"] = "T";
    DiffStatus["Unmerged"] = "U";
    DiffStatus["Unknown"] = "X";
    DiffStatus["Broken"] = "B";
})(DiffStatus || (DiffStatus = {}));
const isStatus = (status) => Object.values(DiffStatus).includes(status) ||
...
function git_diff(options) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((_a = options === null || options === void 0 ? void 0 : options.statuses) === null || _a === void 0 ? void 0 : _a.length) {
            args.push("--diff-filter", options.statuses.join(""));
        }
        if ((_b = options === null || options === void 0 ? void 0 : options.paths) === null || _b === void 0 ? void 0 : _b.length) {
            args.push(...options.paths);
        }
        const { stdout } = yield exec(cmd, args, { cwd: options === null || options === void 0 ? void 0 : options.cwd });
...
const api_DiffStatus = DiffStatus;
function api_diff(options = {}) {
    return api_awaiter(this, void 0, void 0, function* () {
        const { cwd, since, paths, statuses } = options;
        const ref = since ? since : yield getDefaultRef({ cwd });
        const files = yield git_diff({
            cwd,
            ref,
            paths,
            statuses,
...
(() => action_awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        console.log(JSON.stringify(core.getInput("file-path")), JSON.stringify(core.getInput("file-path")
            .split("\n")
            .filter((p) => !!p)));
        console.log(JSON.stringify(core.getInput("file-status")), JSON.stringify(core.getInput("file-status")
            .split("\n")
            .filter((p) => !!p)));
        const filePaths = core.getInput("file-path")
            .split("\n")
            .filter((p) => !!p);
        const fileStatuses = core.getInput("file-status")
            .split("\n")
            .filter((p) => !!p);
        const { ref, files } = yield api_diff({
            cwd,
            since,
            paths: filePaths,
