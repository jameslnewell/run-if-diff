...
async function hasOneOrMoreTags({ cwd }) {
    const { stdout } = await exec("git", ["tag"], { cwd });
    return stdout.trim() !== "";
}
async function getMostRecentTag({ cwd }) {
    const { stdout } = await exec("git", ["describe", "--tags", "--abbrev=0"], { cwd });
    return stdout.split("\n")[0];
}
...
async function getDefaultRef({ cwd, }) {
...
const isStatus = (status) => Object.values(DiffStatus).includes(status) ||
    Object.values(DiffStatus)
        .map((s) => s.toLowerCase())
        .includes(status);
async function git_diff(options) {
    const cmd = "git";
    ...
    const { stdout } = await exec(cmd, args, { cwd: options?.cwd });
...
async function api_diff(options = {}) {
    const { cwd, since, paths, statuses } = options;
    const ref = since ? since : await getDefaultRef({ cwd });
    const files = await git_diff({
        cwd,
        ref,
        paths,
        statuses,
...
(async () => {
    try {
        ...
        const fileStatuses = core.getInput("file-status")
            .split("\n")
            .filter((p) => !!p);
        const { ref, files } = await api_diff({
            cwd,
            since,
            paths: filePaths,
