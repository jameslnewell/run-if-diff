...
export async function getDefaultRef({cwd}: {cwd?: string}): Promise<string> {
...
export type DiffStatusFilter = 'A'|'C'|'D'|'M'|'R'|'T'|'U'|'X'|'B'|'a'|'c'|'d'|'m'|'r'|'t'|'u'|'x'|'b';
...
const isStatus = (status: string): status is DiffStatus => Object.values(DiffStatus).includes(status as DiffStatus)

export async function diff(options?: DiffOptions): Promise<Record<string, DiffStatus>> {
  const cmd = "git";
